/* This code gets the ids meters with complete data for a series of ten day slots. 
...
const int nMetersPerCore = 14847;
...
int main (int argc, char **argv) {
	...
	int nProcsPerNode;
    int slotnum;
    int startIndex, endIndex, nMetersThisCore;
    int corenum;
    ...
    corenum = (slotnum * nProcsPerNode) + procnum;   //zero based
   
   	sprintf(message, "Started...");
	log_info(infoPath, corenum, message);

    startIndex = procnum * nMetersPerCore;
    endIndex = startIndex + nMetersPerCore - 1;
    
    if (endIndex >= nMeters) {
    	endIndex = nMeters - 1;
    }
    nMetersThisCore = endIndex - startIndex + 1;
	...
	long int startWinIndex = round(abs(difftime(start_win_time, mintime)) / 60.0 / 60.0 * 2.0) + (slotnum * 48 * 7);
	long int endWinIndex = startWinIndex + 512 - 1;
	...
	double max_time_range_secs = abs(difftime(maxtime,mintime));		
	...
	int * totalSteps; 
	totalSteps = (int*)calloc(nMetersThisCore, sizeof(int));
	
	double * totalReading; 
	totalReading = (double*)calloc(nMetersThisCore, sizeof(double));
	...
	for (int meterIndex=startIndex; meterIndex<=endIndex; meterIndex++) {
		...
		sprintf(message, "%li/%i/%li", startIndex, meterIndex, endIndex);
		log_info(infoPath, corenum, message);
		...
		if (input_file != NULL) {
			...
			while (fscanf(input_file,"%s %s %s %s %lf %lf %lf \n",&rdate,&rtime,&meterId,&customerId,&reading,&lead,&lag) != EOF) {
				...
				if (timeIndex > max_num_intervals) {
				
					sprintf(message, "**** meterIndex: %li, timeIndex: %li, max_num_intervals: %li, reading: %lf", meterIndex, timeIndex, max_num_intervals, reading);
					log_info(infoPath, corenum, message);			
				}
			...
			totalSteps[meterIndex -startIndex] = totalInWin;
			totalReading[meterIndex - startIndex] = totalWinReading;
		}
		free(reading_list);
	...
	for (int i=0; i<nMetersThisCore; i++) {
	
		if ((totalSteps[i]==winIndexSize) && (totalReading[i]!=0.0)) {
	 		fprintf(output_file, "%i   %i  \n", i + startIndex, slotnum);
		}	
	}	
	fclose(output_file);	
